...
import {listarCitas, listarVeterinarios, guardarCita, findVetById} from "../server/Server";
...
function validarCitas(){
    ...
    for(var i=0;i<citas.length;i++){
       if(citas[i].hora === hora && citas[i].fecha === fecha && citas[i].nombreveterinario === vetNombre) {
        citaExiste = true;
        break;
       }
       else{citaExiste = false;
      }
...
{/* onChange={ () => { this.bChange(); this.hChange(); } }> */}
        </Form.Group>  
    ...
    onClick={ev =>{ev.preventDefault();
     try{encontarPorNombre(vetNombre.toLowerCase());
      validarCitas();
      if(citaExiste){  alert("No puede agendar esta cita." +
      " Este veterinario ya tiene una cita agendada para las " + hora +".");}
     else{  let cajas = {
      fecha:fecha,
      hora:hora,
